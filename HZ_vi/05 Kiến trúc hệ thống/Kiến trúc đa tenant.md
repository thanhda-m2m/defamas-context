...
1. **Mỗi tenant có database riêng** — `{tenantCode}_db` (ví dụ: `ppes_db`).
...
| ------------ | ------------------- | ------ | -------------------------------------------- |
| `tenantCode` | Đoạn đầu URL        | `ppes` | Phân tích `$_SERVER['REQUEST_URI']`          |
...
```mermaid
flowchart TD
    A["Admin điền form buscomps (loginid, tên công ty, bkid tự sinh)"] --> B["saveData() trong buscomps.php"]
